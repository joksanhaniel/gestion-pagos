...
$fees = $conn->query("SELECT ef.*,s.name as sname,s.id_no,concat(c.prestamo,' - ',c.level) as `class` FROM clients_ef_list ef inner join clients s on s.id = ef.clients_id inner join prestamos c on c.id = ef.prestamo_id  where ef.id = {$_GET['ef_id']}");
...
<div class="container-fluid">
	...
	<div class="flex">
		<div class="w-50">
			<p>No de Prestamo: <b><?php echo $ef_no ?></b></p>
			<p>Cliente: <b><?php echo ucwords($sname) ?></b></p>
			<p>Prestamo/Tipo: <b><?php echo $class ?></b></p>
		</div>
		<?php if ($_GET['pid'] > 0) : ?>
			<div class="w-50">
		...
		<tr>
			<td width="50%">
				...
				<table width="100%">
					...
					<?php
					$cfees = $conn->query("SELECT * FROM fees where prestamo_id = $prestamo_id");
					$ftotal = 0;
